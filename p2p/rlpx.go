...
func ToECDSAPub(pub []byte) *ecdsa.PublicKey {
    ...
    // Use decred's parser to handle the public key bytes
    pubKey, err := secp256k1.ParsePubKey(pub)
    if err != nil {
        return nil
    }
    
    // Construct ecdsa.PublicKey from the parsed key's coordinates
...
recoveredPubKey, _, err := dcrdecdsa.RecoverCompact(compactSig, signedMsg)
if err != nil {
    return nil, err
}
remoteRandomPub := recoveredPubKey.SerializeUncompressed()
