...
func CheckECDSASignature(message []byte, pubKeyStr, signatureStr string) (bool, error) {
	...
	compactSig[0] = signature[64] + 27 // V at front, add 27 for compact format
	copy(compactSig[1:33], signature[0:32])   // R
	copy(compactSig[33:65], signature[32:64]) // S

	recoveredPubKey, _, err := dcrdecdsa.RecoverCompact(compactSig, message)
	if err != nil {
		return false, err
	}
	recPubKey := recoveredPubKey.SerializeUncompressed()
...
func (p *ChangeTssECDSAPubKeyMethod) ReceiveBlock(context vm_context.AccountVmContext, sendBlock *nom.AccountBlock) ([]*nom.AccountBlock, error) {
	...
	// Parse the compressed public key and decompress it
	parsedPubKey, err := secp256k1.ParsePubKey(pubKey)
	if err != nil {
    return nil, err
	}
	// Serialize as uncompressed (65 bytes: 0x04 || X || Y)
	dPubKeyBytes := parsedPubKey.SerializeUncompressed()		
